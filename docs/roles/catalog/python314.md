...
Manages Python 3.14, a general-purpose, high-level programming language. Supports
...
| Action | Description |
|---|---|
| `on` | Install or upgrade Python 3.14 |
| `off` | Uninstall Python 3.14 |
| `download` | Download installer to storage |
| `copy` | Copy installer to remote temp (no install) |
| `info` | Report installation state |
| `is_present` | Assert that Python 3.14 is installed |

---

## Configuration

Python 3.14 is installed with the following defaults:
...
- Installation location: `%ProgramFiles%\Python314`
