...
| `win_workman_action_default` | `"on"` | Default action when a task string has no action token |
...
| Variable | Source |
|---|---|
| `win_workman_action` | `argv[1]` of the task string (or `win_workman_action_default`) |
